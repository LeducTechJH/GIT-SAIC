...
function setEditManager ( objName, editManagerType, format, formName, valueField, entry )
{
  ...
  /*if ( format.toString().match(/^\d{3}-\d{2}-\d{4}$/) )
  {
	 alert("objName: " + objName + "\n" + "editManagerType: " + editManagerType + "\n" + "valueField: " + valueField);

	 alert("pObj: " + pObj.name);

	 var element = document.getElementsByName(pObj.name);
...
{
  var inputs = getInputs ( );
  //Create a new imput element after getting the inputs.
  //This base input element object will be cloned.
  //This will allow the cloned input field to obfuscate the original social security fields.
  ...
  for ( var i = 0; i < inputs.length; i++ )
  {
    ...
    if ( inputName != null && inputName.length > 0 )
    {
      ...
      if ( panelObject )
      {
        ...
        //During the input fields parsing iteration we check for fields that have a social security pattern.
        //If one is found we change the styling of the field to the class input_opaque defined in jdp.css.
        //This will hide the social security field. Then we clone the input defined at the start of the init() function.
        //We give the cloned input all the properties and values of the social security field, but we replace,
        //the first five digits with hypens for ***-**-.
        if ( input.value.toString().match(/^\d{3}-\d{2}-\d{4}$/) )
        {

         //Change the styling class to disappear the input.
    	 input.className="input_opaque";

    	 //Clone the plain input element created above.
    	 var clone = newInput.cloneNode(true);

    	 //Give the clone object the styling of the normal input fields.
    	 clone.className="input_normal";

    	 //Add executable function to the clone object when the mouse is on top of the element.
    	 //The function will hide the clone field (that has the SSN obfuscated) and show the real SSN field.
    	 //See selectOrig() bellow.
      	 clone.onmouseover = selectOrig;

      	 //Copy the original field x & y positions to the clone field.
      	 //This will draw the clone field element directly above the normal SSN input field.
      	 //Also copy the size attributes (width).
      	 if (input.style.getAttribute) //Check for style.getAttribute function support. (true for IE)
      	 {
      		 var top = input.style.getAttribute ("top");
      		 var left = input.style.getAttribute ("left");
      		 var width = input.style.getAttribute ("width");
      		 clone.style.setAttribute('top', top);
      		 clone.style.setAttribute('left', left);
      		 clone.style.setAttribute('width', width);
      	 }
      	 else //Standards compliant browsers defaults to getPropertyValue.
      	 {
      		 var top = input.style.getPropertyValue ("top");
      		 var left = input.style.getPropertyValue ("left");
      		 var width = input.style.getPropertyValue ("width");
      		 clone.style.setProperty('top', top, "");
      		 clone.style.setProperty('left', left, "");
      		 clone.style.setProperty('width', width, "");
      	 }

      	 //The id property of the clone input element is named after the cloned input plus "_CLONE" concatenated at the end.
      	 //This will be used to later retrieve the ID of the element and filter the string to select between the original and the cloned.
      	 clone.setAttribute("id", input.id + "_CLONE" );
      	 //We do the same for the name property. All elements in the JDP must have the name set.
      	 clone.setAttribute("name", input.id + "_CLONE" );
      	 //Here we write the value of the cloned field. We use the las 4 digits of the original input field value and fill the rest with the mask.
      	 clone.value = "***-**-" + input.value.toString().substring(7, 11);

      	 //Finaly draw the clone element in to the page if its not a hidden field.
...
function selectOrig ( )
{
	//We take the name of the input element that the mouse is passing over.
	var elemName = this.name;

	//Now we replace the name variable value of the input element the mouse is passing over with the same name minus the part that is written
	//"_CLONE" in the string. Effectively replacing the variable with the value of the name of the original input element that is hidden.
	elemName = elemName.substr(0, elemName.indexOf("_CLONE"));


	var field = document.getElementById(elemName);
	this.className="input_opaque";
	field.className="input_normal";
	field.focus();
	field.onblur = reMask;
...
{
	this.className="input_opaque";
	var inp = document.getElementById(this.name + "_CLONE");
	inp.className="input_normal";
}
