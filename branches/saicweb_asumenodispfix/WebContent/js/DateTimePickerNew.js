...
{ 
  ...
  if ( hasDateInFormat )
  {
	  ...
	  html += xTR;
	  html += "<tr class='dpTitleTR' style='height: 25;'>";
	  if ( editYear )
	  {
	    html += "<td colspan='4'>Pass to year:</td>";
	    html += "<td colspan='2'>" + DIV_title_year + "<input name='baseYear' id='baseYear' tabindex=1 type='text' maxLength=4  style='height: 20; width:35; text-align:right;'/>" + xSPAN + "</td>";
	  }
	  else
	  {
	    html += "<td colspan='6'>&nbsp;&nbsp;&nbsp;To change year --></td>";
	  }
...
function getButtonCode(dateFieldName, dateVal, adjust, label)
{
  ...
  return "<button class='dpButton' onClick='refreshDatePicker(\"" + dateFieldName + "\", " + newYear + ", " + newMonth + ", " + date + ", " + hour + ", " + minutes + ");'>" + label + "</button>";
}
...
{ 
  if ( editYear )
  {
    ...
    {
      editYear = false;
      var existDate = getFieldDate ( dateString );
       
      if ( confirm ( "You realy want to pass to year " + elem.value + " ?" ) )
      {
        if ( elem ) refreshDatePicker(dateFieldName, elem.value, existDate.getMonth ( ), existDate.getDate ( ), existDate.getHours ( ), existDate.getMinutes ( ) ); 
      }
      else
      {
        refreshDatePicker(dateFieldName, existDate.getFullYear ( ), existDate.getMonth ( ), existDate.getDate ( ), existDate.getHours ( ), existDate.getMinutes ( ) );  
      }
    }
    else
    {
