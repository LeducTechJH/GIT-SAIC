	...
	/**  */
	  ...
	  protected void step0 ( ParameterSet ps )
	  {
		  try
		  {
			  ...
			  //System.setProperty("http.proxyHost", "10.200.255.1");
			  //System.setProperty("http.proxyPort", "8080");
			  //System.setProperty("https.proxyHost", "10.200.255.1");
	  ...
	  protected void step1 ( ParameterSet ps )
	  {
		  ...
		  //Overrides bad WSDL config when schemaLocation and soap:address location in WSDL do not reflect the new domain.
		  requestContext.put(bind.ENDPOINT_ADDRESS_PROPERTY, "https://wse1.lumapr.com:443/soa-infra/services/PUBPROD/Account_Inquiry_Prod/bpelprocess2_client_ep");
//test		  requestContext.put(bind.PASSWORD_PROPERTY, "deptfam01");
//PROD		  requestContext.put(bind.PASSWORD_PROPERTY, "F@m1l1@prd");
		  ...
		  if (checkws.matches(regex) )
		  {
			  ...
			  if (Compare.gt(this.app.form("SERV_NUCLEO_SERV_FRM").getItem("CANTIDAD_AUTO"), 1000)) 
			  {
				  app.form("SERV_NUCLEO_SERV_FRM").setItem("CANTIDAD_AUTO", 1000);					
			  }
			  ...
			  Iterator<CMWSSPaymentHistory> it1 =cmwssPaymentHistory.value.iterator();
			  int n=0;
			  int t=0;
			  while(it1.hasNext())
			  {
				  Iterator<PayList> payit =cmwssPaymentHistory.value.get(n).getPayList().iterator();
				  while(payit.hasNext())
				  {
					  t++;
					  payit.next();
			  ...
			  while(it.hasNext())
			  {
				  ...
				  while(payit1.hasNext())
				  {
					  ...
					  app.form("SERV_NUCLEO_SERV_FRM").setItem("FCH_ULT_PAGO_"+n1,cmwssPaymentHistory.value.get(0).getPayList().get(t).getPayDate());
					  app.form("SERV_NUCLEO_SERV_FRM").setItem("ULT_PAGO_"+n1, cmwssPaymentHistory.value.get(0).getPayList().get(t).getPayAmount());
					  payit1.next();
					  n1++;
					  t = t+1;
				  }
			  ...
			  while(premit.hasNext())
			  {
				  ...
				  {
					  servact++;
					  app.form("SERV_NUCLEO_SERV_FRM").setItem("DIRECCION",premList.value.get(pren).getPremiseInfo());
				  }
				  pren++;
				  premit.next();
			  }		  
				  if(Compare.equals(multiplePremise.value, "Y")&&(Compare.gt(servact, 1)))
				  {
					  this.app.setError("0", "Cuenta no cualifica. Tiene multiples predios.");
					  app.form("SERV_NUCLEO_SERV_FRM").setItem("FLAG_MULT","X");
					  return;
				  }								  
			  }
		  else
