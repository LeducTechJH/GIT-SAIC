...
extends ADL
{
	...
	protected void step1 ( ParameterSet ps )
	{
		...
		{        
			...
			case 1:
			{
				if (Compare.equals(this.dm.getFieldString ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "LIC_DPT_FAM_FLG" ), "X") ) 
 				   {
					if (( Compare.le ( this.dm.getFieldDate ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "HASTA_LIC" ), this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem("MESPROYECTADO") ))   )
					{
						doc_activC = false;
					}
					else
				...
				else if   (Compare.equals(this.dm.getFieldString ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "PROV_DPT_FAM_FLG" ), "X") )		
				   {
					if (( Compare.le ( this.dm.getFieldDate ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "HASTA_PROV" ), this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem("MESPROYECTADO") ))   )
					{
						doc_activC = false;
					}
					else
			...
			case 2:
			{
				...
				{
					if (( Compare.le ( this.dm.getFieldDate ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "HASTA_COND" ), this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem("MESPROYECTADO") ))  ||
							(  Compare.le ( this.dm.getFieldDate ( this.dm.get ( "PROVEEDORES_TBL_DSD" ), "HASTA_SALU" ), this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem("MESPROYECTADO") )) )
					{
						doc_activI = false;
					}
					else
		...
		this.dm.setField ( this.dm.get ( "CONTRATO_CUIDO_DSD" ), "SEGURO_SOCIAL_CLIENTE", this.app.form ( "CONTRATO_PROYECCION_FR" ).getDoubleItem ( "SEG_SOC_CLIENTE" ));
		this.dm.setField ( this.dm.get ( "CONTRATO_CUIDO_DSD" ), "DESDE", this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem ( "DESDE" ));
		this.dm.setField ( this.dm.get ( "CONTRATO_CUIDO_DSD" ), "HASTA", this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem ( "HASTA" ));
		...
		if (Compare.equals(CONTRATOS_CUIDO_STATUS, 0))
		{
			...
			flg_necespecial = this.dm.getFieldString( this.dm.get ( "CONTRATO_CUIDO_DSD" ), "NECESIDAD_ESP" );
		}
		else
		{
	...
	protected void step2 ( ParameterSet ps )
	{
		...
		double dias_ajust = this.app.form ( "CONTRATO_PROYECCION_FR" ).getDoubleItem ( "DIAS_AJUST" );
		double rate_comp = 0;
		double rate_part = 0;
		...
		if ( Compare.equals ( STATUS_ID, 0 ))
		{
			...
			EDAD = DateUtil.monthsBetween ( this.app.form ( "CONTRATO_PROYECCION_FR" ).getDateItem ( "MESPROYECTADO" ), this.dm.getFieldDate ( this.dm.get ( "CLIENTES2_DSD" ), "FCH_NACIMIENTO" ) ) / 12;
			if ( Compare.ge( EDAD, 0 ) &&  ( Compare.lt(  EDAD, 18 ) ) && Compare.equals(flg_necespecial, "X") )  
			{
				categoria = "NECESIDAD ESPECIAL";
			}
			else if  ( Compare.lt(  EDAD, 3 ) )  
			{
				categoria = "INFANTES 0-2 CON 11 MESES";
			}
			else if ( Compare.ge( EDAD, 3 ) &&  ( Compare.lt(  EDAD, 5 ) ) )  
			{
				categoria = "PRE_ESCOLARES 3-4 CON 11 MESES";
			}
			else if ( Compare.ge( EDAD, 5 ) &&  ( Compare.lt(  EDAD, 13 ) ) )  
			{
				categoria = "ESCOLARES 5-12 CON 11 MESES";
			}
			else 
			{
				this.app.setError("0", "Este miembro sobre pasa la edad determinada para el beneficio de cuido del menores.");
			...
			if ( Compare.equals ( STATUS1_ID, 0 ) ) 
			{
				...
				double costo_comp = Double.valueOf(df.format(this.app.form ( "CONTRATO_PROYECCION_FR" ).getDoubleItem("TIEMPO_COMP")));
				double costo_part = Double.valueOf(df.format(this.app.form ( "CONTRATO_PROYECCION_FR" ).getDoubleItem("TIEMPO_PART")));
				...
				else 
				{
					...
					{
						this.app.setError("0", "Las tarifas calculadas sobrepasan las establecidas en el contrato.");
					}
					else
					{
						if(Compare.equals(tipo, 1) || Compare.equals(tipo, 3))
						{
							pago = MathService.plus(costo_comp,costo_part);
							this.app.form ( "CONTRATO_PROYECCION_FR" ).setItem ( "PAGO", TruncService.round (pago)); 
							this.app.form ( "CONTRATO_PROYECCION_FR" ).setItem ( "ID_TECNICO",this.app.ID_TECNICO_GBL);
						}
						else
						{
							pago = MathService.plus(rate_comp * dias_asis,rate_part * dias_asis_part);
							if(Compare.gt(this.app.form ( "CONTRATO_PROYECCION_FR" ).getDoubleItem("AJUSTE"),0 ))
							{
								...
								if(Compare.gt((dias_ajust*ajuste), pago))
								{
									this.app.setError("0", "Cantidad ajustar sobrepasa el pago.");
								}
								else
								{
									if(Compare.equals(this.app.form ( "CONTRATO_PROYECCION_FR" ).getStringItem("ESNEGATIVO"),"D" ))
									{
										pago = MathService.minus(pago, dias_ajust*ajuste);
									}
									else if(Compare.equals(this.app.form ( "CONTRATO_PROYECCION_FR" ).getStringItem("ESNEGATIVO"),"A" ))
									{
										pago = MathService.plus(pago, dias_ajust*ajuste);
									}
								}
							}
							this.app.form ( "CONTRATO_PROYECCION_FR" ).setItem ( "PAGO", TruncService.round (pago)); 
