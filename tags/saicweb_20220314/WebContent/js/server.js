...
function getRequestResponse ( )
{
  ...
  if ( xmlHttp && xmlHttp.readyState == XML_HTTP_DONE )
  {
    try
    {
      if ( xmlHttp.status == HTTP_STATUS_OK )
      {
        ...
        var n = response.indexOf('model="SEL_RMO_OFICINA_ALL_FR"');
        var m = response.indexOf('name="RMO_OFICNA"');
        if(n != -1 && m == -1)
        {
        	applicationKeyUpEventHandler("PICKLIST");
        }
        
        n = response.indexOf('<JDPPickList command="JDPPickList">');
        m = response.indexOf('<parameter name="field">RMO_OFICNA');
        if(n != -1 && m != -1)
        {
        	fillRMO(response);
        }
  ...
  function fillRMO(response)
  {
	  var start = response.indexOf('<tr');
	  var end = response.lastIndexOf('</tr');
	  var tbody = response.substring(start,end+4);
	  tbody = tbody.replace(/onclick/g,'onmouseover="changeBackColor(this,1);" onmouseout="changeBackColor(this,0)" onclick');
	  tbody = tbody.replace(/returnData/g,'submitData');
	  //tbody = tbody.replace(/returnData/g,'parent.returnData');
	  var ele = parent.document.getElementById("RMOOficina");
	  ele.innerHTML = tbody;
  }
...
function sendRequest ( url, request )
{
    ...
    var n = request.indexOf('command=NOTIFYINSTANCE');
    var m = request.indexOf('model=SEL_RMO_OFICINA_ALL_FR');
    
    if(n != -1 && m != -1)
    {
    	n = request.indexOf('&value=') + 7;
        m = request.indexOf('&changed=');
        
        var ascii = request.substring(n,m);
        ...
        if (ascii != "")
        	{
        		...
        		var rows = parent.document.getElementById("RMOOficina").rows;
        		...
        		for(var i=0;i<rows.length;i++)
        		{
        			...
        			if (td.innerHTML == txt)
        				{
        					var ofi = rows[i].getElementsByTagName("td")[3].innerHTML;
        		...
        		{
        			txt = txt + String.fromCharCode(value);
        		}
