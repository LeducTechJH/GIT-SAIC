...
String SAIC_Usr = "SAIC";
String SAIC_Pwd = "ora12cr2";
String SAIC_DBStr = "jdbc:oracle:thin:@10.200.10.50:1521:SAICQC";
Class.forName(SAIC_Drvr);
Connection con = DriverManager.getConnection(SAIC_DBStr,SAIC_Usr,SAIC_Pwd);
String qry = "SELECT TO_CHAR(aj.mes_porciento,'MONTH YYYY','nls_date_language=SPANISH') AS AJUSTE, "+
					  "to_char(aj.porciento,'0.0000')                              AS PORCIENTO, "+
					  "nvl((select aviso from aviso) ,' ')       AS AVISO "+
					  "FROM ajuste_pan aj "+
					  "WHERE TO_CHAR(aj.mes_porciento,'MMYYYY') = TO_CHAR(sysdate, 'MMYYYY') ";
try 
{
    ...
    while (rs.next()) 
    {
    	String ajusteqry = rs.getString("AJUSTE");
    	double porqry = rs.getDouble("PORCIENTO");
    	DecimalFormat df = new DecimalFormat("0.0000");
    	String avisoqry = rs.getString("AVISO");
    	ajuste = ajusteqry;
    	porciento = df.format(porqry);
    	aviso = avisoqry;
    }
    st.close(); 
}
...
{
	message = "<div style=\"width:90%; font:20px; aligh:center;\"><b>RECORDATORIO: <font color=red>AL ACCEDER ESTE SISTEMA, USTED ESTARÁ APLICANDO CAMBIOS A DOCUMENTOS OFICIALES</font></b><div align=\"left\">  </br><b><font color=red> EL AJUSTE PORCENTUAL PARA EL MES DE " + ajuste+ " ES DE "+porciento+".</b></br><b><font color=red>"+ aviso +"</b></br>";
}
%>
<html>
 ...
 */
 ...
 {	
			...
			//Check if the user is blocked
				 $.getJSON("http://10.200.10.79/RMS/RMS.srvc/SampleService?op=checkifblock&cod_seguridad=" + userName ,
				function(isBlockResponse) 
				{
					//if the response value is true, let the user know it is blocked in SAIC and get the samples.
					if (isBlockResponse.toString().toLowerCase() == "true")
					{				
						alert("Su acceso a SAIC ha sido bloqueado debido a muestras pendientes por completar en RMS.");
						
						//Getting the user samples
						$.getJSON("http://10.200.10.79/RMS/RMS.srvc/SampleService?op=getsamples&cod_seguridad=" + userName,
						function(getSamplesResponse) 
						{
							if (getSamplesResponse.length > 0){	
								//Redirecting to the sample url
								window.location = getSamplesResponse[0].SampleURL+"&fs=true";
							}
							else{
								alert('No sample found for specified user.');
								return false;
							}
						});
						return true;
						...
						alert('No user found or user is not blocked.');
						return false; 
					}
				}); 
			}
			else {
				alert('Favor especificar nombre de usuario.');
			}
 }
...
function applicationResponseHandler ( root )
{
  ...
  else if ( command == "failure" )
  {
    ...
    if ( message != null )
    {
      ...
      {
    	var userName = getElementValue ( "userName" ).replace(/^\s+|\s+$/g,"");
    	var isLock =  checkRMS_LOCK(userName);
    	  if (isLock)
    		  {
    		  //No alert
    		  }
...
<p align="center>">
				<jsp:setProperty /><select id="userNames" style="width:130px; height:16px; font:11px Arial;" onChange="document.getElementById ( 'userName' ).value = this.value;">
  <option value=" ">&nbsp;</option>
...
<table cellpadding="1" cellspacing="1" style="background:#FFCC33; border: 1px solid #00A0FF; background-color: Aqua">

<tr valign="bottom" style="font:15px Arial; color: #005080; height: 117px">
<td colspan="2" valign="top" style="height: 97px; ">
